#include <SPI.h>
#include <RF24.h>

// Pin definitions for nRF24L01+
#define CE_PIN 9
#define CSN_PIN 10

// Radio pipe addresses for the 2 nodes to communicate
const uint64_t pipes[2] = { 0xF0F0F0F0E1LL, 0xF0F0F0F0D2LL };

// Initialize RF24 object
RF24 radio(CE_PIN, CSN_PIN);

// Buffer for serial communication
const int BUFFER_SIZE = 64;
char serialBuffer[BUFFER_SIZE];
int bufferIndex = 0;

void setup() {
    // Initialize serial communication
    Serial.begin(115200);
    
    // Initialize radio communication
    if (!radio.begin()) {
        Serial.println("Radio hardware not responding!");
        while (1);
    }
    
    // Radio configuration
    radio.setPALevel(RF24_PA_LOW);      
    radio.setChannel(76);               
    radio.setDataRate(RF24_250KBPS);    
    radio.enableDynamicPayloads();      
    radio.setRetries(5, 15);            
    
    radio.openWritingPipe(pipes[0]);
    radio.openReadingPipe(1, pipes[1]);
    
    radio.startListening();
    
    Serial.println("Ground Station Ready!");
    Serial.println("Waiting for commands from Python interface...");
}

void loop() {
    // Read commands from Python
    while (Serial.available() > 0) {
        char inChar = Serial.read();
        
        if (inChar == '\n') {
            serialBuffer[bufferIndex] = '\0';  // Null terminate the string
            processSerialCommand(serialBuffer);
            bufferIndex = 0;  // Reset buffer
        } else if (bufferIndex < BUFFER_SIZE - 1) {
            serialBuffer[bufferIndex++] = inChar;
        }
    }
}

void processSerialCommand(const char* command) {
    radio.stopListening();
    
    // Send command through radio
    bool success = radio.write(command, strlen(command) + 1);
    
    radio.startListening();
    
    // Echo command back to Python for confirmation
    if (success) {
        Serial.print("CMD_SENT:");
        Serial.println(command);
    } else {
        Serial.println("CMD_FAILED");
    }
} 